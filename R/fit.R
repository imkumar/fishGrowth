...
vbgf <- function(data) {
    ...
    Lobs <- ag_cod$length
    div <- ag_cod$NAFOdiv
    divs <- as.factor(div)%>%as.numeric-1
    ...
    rname = c("rlog_Linf","rlog_k","rlog_a","rlog_b","rlog_Linfc","rlog_kc","rlog_ac","rlog_bc")

    m17.obj <- MakeADFun(tmb.data,parameters,random=rname,DLL="m17",inner.control=list(maxit=100,trace=T))
    m17.obj$gr(m17.obj$par)

    m17.opt<-nlminb(m17.obj$par,m17.obj$fn,m17.obj$gr,lower=lower,upper=upper,control = list(trace=0,iter.max=5000,eval.max=10000))

    m17.opt$message
    m17.obj$gr(m17.opt$par)

    m17.rep = m17.obj$report()

    m17.sd.rep<-sdreport(m17.obj)
